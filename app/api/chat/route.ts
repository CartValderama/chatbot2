...
import type { ChatRequest, ChatResponse } from "@/lib/groq/types";
...
export async function POST(request: NextRequest) {
  try {
    ...
    // 8. PARSE TEXT-BASED FUNCTION CALLS (fallback for models that generate <function=name></function>)
    if (!assistantMessage?.tool_calls && assistantMessage?.content) {
      const functionMatches = Array.from(assistantMessage.content.matchAll(/<function=(\w+)><\/function>/g));
      const parsedToolCalls = functionMatches.map((match, index) => ({
    ...
    while (assistantMessage?.tool_calls && iteration < maxIterations) {
      ...
      const toolResults = await Promise.all(
        assistantMessage.tool_calls.map(
          async (toolCall: ChatCompletionMessageToolCall) => {
            ...
            console.log(
              `[Chat API] Function ${functionName} returned:`,
      ...
      console.log(
        `[Chat API] Adding ${toolResults.length} tool results to message history`
      );
    ...
    const botResponseText =
      ...
      "Jeg kunne ikke generere et svar. Prøv igjen.";
    ...
    // 11. RETURNER SUKSESS
    console.log(
      `[Chat API] Successfully processed message for userId ${userId}`
    );
